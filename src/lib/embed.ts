...
export const EMBED_SERVERS: EmbedServer[] = [
  // --- TURKCE ALTYAZILI (tr_sub) ---
  {
    ...
    getUrl: ({ mediaType, tmdbId, season = 1, episode = 1, title = "" }) =>
      mediaType === "movie"
        ...
        : `/api/player/dizi-embed?title=${encodeURIComponent(title)}&season=${season}&episode=${episode}&mediaType=tv&tmdbId=${tmdbId}&lang=tr_sub`,
  },
  {
    id: "hdf-player-sub",
    ...
    getUrl: ({ mediaType, tmdbId, season = 1, episode = 1, title = "" }) =>
      mediaType === "movie"
        ...
        : `/api/player/hdf-embed?title=${encodeURIComponent(title)}&season=${season}&episode=${episode}&mediaType=tv&tmdbId=${tmdbId}&lang=tr_sub`,
  },
  {
    id: "vidlink-pro",
    name: "VidLink Pro",
  ...
  // --- TURKCE DUBLAJ (tr_dub) ---
  {
    ...
    getUrl: ({ mediaType, tmdbId, season = 1, episode = 1, title = "" }) =>
      mediaType === "movie"
        ? `/api/player/atom-embed?title=${encodeURIComponent(title)}&tmdbId=${tmdbId}`
        : `/api/player/dizi-embed?title=${encodeURIComponent(title)}&season=${season}&episode=${episode}&mediaType=tv&tmdbId=${tmdbId}&lang=tr_dub`,
  },
  {
    id: "hdf-player-dub",
    name: "🔥 HDFilmCehennemi",
    ...
    getUrl: ({ mediaType, tmdbId, season = 1, episode = 1, title = "" }) =>
      mediaType === "movie"
        ...
        : `/api/player/hdf-embed?title=${encodeURIComponent(title)}&season=${season}&episode=${episode}&mediaType=tv&tmdbId=${tmdbId}&lang=tr_dub`,
  },
  {
    id: "dub-multiembed",
