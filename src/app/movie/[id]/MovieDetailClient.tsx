...
export default function MovieDetailClient({ movie }: { movie: MediaDetail }) {
  const { user } = useAuth();
  ...
  // Watchlist & Watched durumunu getir
  useEffect(() => {
    if (!user) return;

    fetch(`/api/watchlist?mediaType=movie&tmdbId=${movie.id}`)
      .then((r) => r.json())
      .then((d) => setInWatchlist(d.inWatchlist))
      .catch(() => {});

    fetch(`/api/watched?mediaType=movie&tmdbId=${movie.id}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.watched && d.watched.length > 0) {
          setIsWatched(true);
        }
      })
      .catch(() => {});
  ...
  const toggleWatchlist = async () => {
    ...
    try {
      if (inWatchlist) {
        await fetch(`/api/watchlist?mediaType=movie&tmdbId=${movie.id}`, { method: "DELETE" });
  ...
  const toggleWatched = async () => {
    if (!user) {
      alert("İzlendi olarak işaretlemek için lütfen giriş yapın.");
      return;
    }
    ...
    <div className="min-h-screen pb-20 space-y-12">
      ...
      <div className="relative w-full overflow-hidden bg-[#06070b]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-16 pb-12 flex flex-col sm:flex-row gap-6 sm:gap-8 items-center sm:items-end">
          {/* Floating Movie Poster (Mobile & Desktop) */}
          ...
          <div className="flex-1 space-y-4 max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 rounded-lg bg-red-600 text-white text-xs font-black uppercase tracking-wider shadow-md shadow-red-600/30">
                Film
              </span>
              ...
              <span className="px-2.5 py-0.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-bold flex items-center gap-1">
                ...
                {movie.vote_average ? movie.vote_average.toFixed(1) : "8.0"} TMDB
              </span>
              {year && (
              ...
              {movie.runtime && (
                <span className="px-2.5 py-0.5 rounded-lg bg-white/5 border border-white/10 text-gray-300 text-xs font-semibold flex items-center gap-1">
                  ...
                  {runtimeHours ? `${runtimeHours}s ` : ""}{runtimeMinutes}dk
                </span>
              )}
            </div>
            ...
            {/* Tagline */}
            {movie.tagline && (
              <p className="text-sm sm:text-base italic text-gray-300 font-medium border-l-2 border-red-500 pl-3">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-3">
              ...
              >
                ...
                <span>{isWatched ? "İzlendi Olarak İşaretlendi" : "İzledim"}</span>
              </button>
              ...
              >
                ...
                <span>{inWatchlist ? "Listemden Çıkar" : "Listeme Ekle"}</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div id="player-section" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 pt-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white tracking-tight">
              1080p Kesintisiz Oynatıcı & Sinema Modu
            </h2>
          </div>
          <span className="text-xs text-gray-400 hidden sm:inline">
            Işıkları kapatmak için sağ üstteki <strong>Sinema Modu</strong> butonunu kullanabilirsiniz.
          </span>
        </div>
        ...
        <VideoPlayer
          ...
          tmdbId={movie.id}
          title={movie.title || "Film"}
          directVideoUrl={(movie as any).videoUrl}
          dubUrl={(movie as any).dubUrl}
          subUrl={(movie as any).subUrl}
          genres={movie.genres?.map((g) => g.name).join(", ") || (movie as any).genres}
          originalLanguage={(movie as any).original_language || (movie as any).originalLanguage}
        />
      </div>

      {/* 3. DETAILS, CAST & SIDEBAR */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-card border border-white/10 rounded-2xl p-6 shadow-xl space-y-3">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                ...
                <span>Film Özeti & Konusu</span>
              </h3>
              <p className="text-sm text-gray-300 leading-relaxed font-normal">
                {movie.overview || "Bu film için henüz özet bilgisi eklenmedi."}
              </p>
            </div>
                ...
                <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
                  {movie.credits.cast.slice(0, 10).map((actor) => (
                    <div key={actor.id} className="w-24 shrink-0 text-center space-y-1.5 group">
                      <div className="w-18 h-18 mx-auto rounded-2xl overflow-hidden bg-black/50 border border-white/10 group-hover:border-red-500/50 group-hover:scale-105 transition-all shadow-md">
                        <img
                      ...
                      <p className="text-xs font-bold text-gray-200 group-hover:text-red-400 transition-colors truncate">
                        {actor.name}
                      </p>
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card border border-white/10 rounded-2xl p-5 shadow-xl space-y-3 text-xs">
              ...
              <div className="space-y-2 text-gray-300">
                <div className="flex justify-between">
                  <span className="text-gray-400">Yayın Yılı:</span>
                  <span className="font-semibold text-white">{year || "—"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Süre:</span>
                  <span className="font-semibold text-white">{movie.runtime ? `${movie.runtime} dk` : "—"}</span>
                </div>
                <div className="flex justify-between">
            ...
            <CommentSection mediaType="movie" tmdbId={movie.id} />
          </div>
        </div>
      </div>
    </div>
