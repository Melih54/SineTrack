...
export async function GET(req: Request) {
    ...
    if (tmdbId) {
      ...
      return new NextResponse(
        ...
        <head>
          ...
          <style>
            ...
            .badge {
              ...
              background: rgba(15, 17, 26, 0.9); color: #f59e0b; font-size: 11px; font-weight: bold;
              padding: 5px 14px; border-radius: 9999px; border: 1px solid rgba(245, 158, 11, 0.3);
        ...
        <body>
          <div class="badge">
            <span class="dot"></span>
            <span>HDFilmCehennemi Alternatif Sunucu (${lang === "tr_dub" ? "Türkçe Ses" : "Türkçe Altyazı"})</span>
          </div>
    ...
    return new NextResponse(
      ...
      <body>
        <div class="box">
          ...
          <p><strong>${title} ${!isMovie ? `(${season}. Sezon ${episode}. Bölüm)` : ""}</strong> için kaynak kontrol ediliyor. Lütfen sayfayı yenileyin veya aşağıdaki diğer sunuculardan birini deneyin.</p>
  ...
  const tracksHtml = subtitles
    .map((sub) => {
      const subProxyUrl = `/api/player/stream-proxy?ref=${encodeURIComponent(hdf!.referer)}&url=${encodeURIComponent(sub.file)}`;
...
<head>
  ...
  <title>${title} ${!isMovie ? `- ${season}. Sezon ${episode}. Bölüm` : ""} | HDFilmCehennemi</title>
...
<body>
  <div id="player-container">
    <div id="loader" onclick="startManualPlayback()">
      ...
      <div class="loading-sub" id="loader-sub">${displaySubtitle}${lang === "tr_dub" ? "Türkçe Dublaj" : "Türkçe Altyazı"} • HDFilmCehennemi</div>
